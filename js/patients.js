(function() {
    ...
    window.sendC = (id) => {
        ...
        if(txt) { App.db.ref(`artifacts/${App.config.APP_ID}/patients/${id}/journal`).push({ text: txt, author: 'Dentista', timestamp: new Date().toISOString() }); document.getElementById('c-msg').value = ''; }
    };

    window.askAI = async (id) => {
        const p = App.data.patients.find(x => x.id === id);
        const snaps = await App.db.ref(`artifacts/${App.config.APP_ID}/patients/${id}/journal`).limitToLast(5).once('value');
