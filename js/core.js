(function() {
    ...
    function loadData() {
        ...
        const load = (path, key) => App.db.ref(App.utils.getAdminPath(uid, path)).on('value', s => {
            App.data[key] = [];
            if(s.exists()) s.forEach(c => { let i = c.val(); i.id = c.key; App.data[key].push(i); });
            if(App.currentView === 'dashboard') renderDashboard();
            if(App.currentView === 'patients' && window.renderPatientList) window.renderPatientList();
            if(App.currentView === 'financials' && window.renderStockList) {
                 if(document.getElementById('stock-tb')) window.renderStockList();
                 if(document.getElementById('rec-list')) window.renderReceivablesList();
                 if(document.getElementById('exp-list')) window.renderExpensesList();
            }
        });
        load('patients', 'patients'); load('stock', 'stock'); load('finance/receivable', 'receivables'); load('finance/expenses', 'expenses');
    }
    ...
    function navigateTo(view) {
        ...
        else if (view === 'patients' && window.initPatientView) window.initPatientView(main);
        else if (view === 'financials' && window.initFinanceView) window.initFinanceView(main);
    ...
    function renderDashboard() {
        ...
        document.getElementById('main-content').innerHTML = `
            <div class="p-8 bg-white shadow-2xl rounded-2xl border border-indigo-100">
                ...
                <div class="border p-4 rounded-xl bg-gray-50">
                    ...
                    <button id="save-brain" class="mt-2 bg-indigo-600 text-white px-4 py-1 rounded">Salvar</button>
                </div>
        ...
        document.getElementById('save-brain').onclick = () => { App.db.ref(brainPath).update({ promptDirectives: document.getElementById('brain-input').value }); alert("Salvo!"); };
    }
    ...
    function setupListeners() {
        ...
        form.replaceWith(form.cloneNode(true)); // Limpa listeners
