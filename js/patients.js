(function() {
        ...
        document.getElementById('main-content').innerHTML = `
            <div class="p-4 md:p-8 bg-white shadow-lg rounded-2xl">
                <div class="flex justify-between mb-6 items-center">
                    ...
                    <button onclick="window.openPatientModal()" class="bg-indigo-600 text-white px-4 py-2 rounded shadow text-sm">Novo Paciente</button>
                </div>
    ...
    window.openPatientModal = function(pid = null) {
        ...
        const html = `
            <form id="form-pat" class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                <input type="hidden" id="p-id" value="${pid || ''}">
                <div class="md:col-span-2"><label class="font-bold">Nome</label><input id="p-name" class="w-full border p-2 rounded" value="${p?p.name:''}" required></div>
                ...
                <div class="md:col-span-2"><label class="font-bold">Endereço</label><input id="p-addr" class="w-full border p-2 rounded" value="${p?p.address:''}"></div>
                ...
                <button class="md:col-span-2 bg-green-600 text-white py-3 rounded font-bold mt-2">Salvar Ficha</button>
            </form>`;
        App.utils.openModal(p?'Editar':'Novo', html, 'md:max-w-lg');
    ...
    window.openJournal = function(id) {
        ...
        const html = `
            <div class="bg-indigo-50 p-3 rounded mb-3 flex justify-between text-sm"><div><h3 class="font-bold">${p.name}</h3></div><span class="bg-white px-2 rounded text-xs flex items-center">${p.treatmentType}</span></div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 h-[60vh]">
                <div class="border p-2 rounded bg-white overflow-y-auto" id="j-fin"></div>
                <div class="border p-2 rounded bg-white flex flex-col"><div id="chat" class="flex-grow overflow-y-auto p-2 mb-2 bg-gray-50 rounded"></div><div class="flex gap-2"><input id="c-msg" class="flex-grow border p-2 rounded text-sm" placeholder="Mensagem..."><button onclick="window.sendC('${id}')" class="bg-indigo-600 text-white px-3 rounded"><i class='bx bxs-send'></i></button><button onclick="window.askAI('${id}')" class="bg-purple-600 text-white px-3 rounded"><i class='bx bxs-magic-wand'></i></button></div></div>
            </div>`;
        App.utils.openModal("Prontuário", html, 'md:max-w-6xl');
        
        const finDiv = document.getElementById('j-fin');
        const recs = App.data.receivables.filter(r => r.patientId === id);
        recs.forEach(r => finDiv.innerHTML += `<div class="border-b pb-2 mb-2 text-sm flex justify-between"><span>${r.description}</span><b>${App.utils.formatCurrency(r.amount)}</b></div>`);
        ...
        currentChatRef.limitToLast(50).on('child_added', s => {
            ...
            d.innerHTML = `<strong class="block text-[10px] opacity-70 uppercase">${m.author}</strong>${m.text}`;
    ...
    window.askAI = async (id) => {
        ...
        try {
            ...
            const html = `<div class="text-sm text-gray-600 mb-2">Sugestão da IA:</div><textarea id="ai-res" class="w-full h-32 border p-2 text-sm bg-purple-50 rounded mb-3">${resp}</textarea><div class="flex justify-end gap-2"><button onclick="document.getElementById('ai-over').remove()" class="text-gray-500">Fechar</button><button onclick="useAI()" class="bg-purple-600 text-white px-4 py-2 rounded text-sm font-bold">Usar no Chat</button></div>`;
            const overlay = document.createElement('div'); overlay.id='ai-over'; overlay.className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[2000] p-4'; overlay.innerHTML=`<div class="bg-white p-4 rounded-xl shadow-xl w-full max-w-md">${html}</div>`; document.body.appendChild(overlay);
